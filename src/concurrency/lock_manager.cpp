...
auto LockManager::LockTable(Transaction *txn, LockMode lock_mode, const table_oid_t &oid) -> bool { 
  ...
  for(auto request:lock_request_queue->request_queue_){
    ...
    if(!(request->lock_mode_==LockMode::INTENTION_SHARED ) ||
       request->lock_mode_!=LockMode::SHARED || (lock_mode!=LockMode::EXCLUSIVE && lock_mode!=LockMode::SHARED_INTENTION_EXCLUSIVE)||
       request->lock_mode_!=LockMode::INTENTION_EXCLUSIVE || (lock_mode!=LockMode::EXCLUSIVE && lock_mode!=LockMode::SHARED_INTENTION_EXCLUSIVE) ||
    ...
    InsertOrDeleteTableLockSet(txn,std::make_shared<LockRequest>(request),false);
...
auto LockManager::UnlockTable(Transaction *txn, const table_oid_t &oid) -> bool { 
  ...
  if(!(s_row_lock_set->find(oid)==s_row_lock_set->end() || x_row_lock_set->find(oid)==x_row_lock_set->end() || 
       s_row_lock_set->at(oid).empty() || x_row_lock_set->at(oid).empty())){
    table_lock_map_latch_.unlock();
    txn->SetState(TransactionState::ABORTED);
     ...
     if(lock_request->txn_id_==txn->GetTransactionId() && lock_request->granted_){
      ...
      if(lock_request->lock_mode_==LockMode::SHARED || lock_request->lock_mode_==LockMode::EXCLUSIVE){
        if((txn->GetIsolationLevel()==IsolationLevel::REPEATABLE_READ) ||
            ...
            (txn->GetIsolationLevel()==IsolationLevel::READ_UNCOMMITTED && lock_request->lock_mode_==LockMode::EXCLUSIVE)){
...
auto LockManager::LockRow(Transaction *txn, LockMode lock_mode, const table_oid_t &oid, const RID &rid) -> bool {
  ...
  if(txn->GetIsolationLevel()==IsolationLevel::READ_COMMITTED && txn->GetState()==TransactionState::SHRINKING && lock_mode!=LockMode::INTENTION_SHARED && lock_mode!=LockMode::SHARED){
    txn->SetState(TransactionState::ABORTED);
      throw TransactionAbortException(txn->GetTransactionId(),AbortReason::LOCK_ON_SHRINKING);
  }
  if(txn->GetIsolationLevel()==IsolationLevel::REPEATABLE_READ && txn->GetState()==TransactionState::SHRINKING){
  ...
  if(lock_mode==LockMode::SHARED){
    if(!txn->IsTableExclusiveLocked(oid) && !txn->IsTableIntentionExclusiveLocked(oid) && !txn->IsTableIntentionSharedLocked(oid) &&
...
auto LockManager::HasCycle(txn_id_t *txn_id) -> bool { 
  for(auto const& start_txj_id:txn_set_){
    if(DFS(start_txj_id)){
  ...
  while (enable_cycle_detection_) {
    ...
    {  
      ...
      for(auto &pair :table_lock_map_){
        ...
        for(auto const &request :pair.second->request_queue_){
          if(request->granted_){
